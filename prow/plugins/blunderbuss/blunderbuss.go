...
func handle(ghc githubClient, roc repoownersClient, log *logrus.Entry, reviewerCount, oldReviewCount *int, maxReviewers int, excludeApprovers bool, repo *scallywag.Repo, pr *scallywag.PullRequest) error {
...
func getReviewers(rc reviewersClient, author string, files []scallywag.PullRequestChange, minReviewers int) ([]string, []string, error) {
	authorSet := sets.NewString(scallywag.NormLogin(author))
