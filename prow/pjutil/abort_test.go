...
import (
	...
	prowjobv1 "k8s.io/test-infra/prow/kube"
)
...
func TestTerminateOlderPresubmitJobs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			aborter := NewProwJobAborter(pjc, logrus.NewEntry(logrus.StandardLogger()))
			cleanedupPJs := sets.NewString()
			err := aborter.TerminateOlderPresubmitJobs(tc.pjs, func(pj prowjobv1.ProwJob) error {
